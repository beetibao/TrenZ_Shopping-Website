    ...
    try {
      ...
      if (!username || !oldPassword || !newPassword) {
        return res.render("changePassword", {
          status: { error: "Tất cả các trường là bắt buộc." },
        });
      }
      ...
      if (!user) {
        return res.render("changePassword", {
          status: { error: "Không tìm thấy người dùng." },
        });
      }
      ...
      if (!isMatch) {
        return res.render("changePassword", {
          status: { error: "Mật khẩu cũ không chính xác." },
        });
      }
      ...
      res.render("changePassword", {
        status: { success: "Mật khẩu đã được thay đổi thành công." },
      });
    } catch (error) {
